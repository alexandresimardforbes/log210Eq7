...
class LocalsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create local" do
    assert_difference('Local.count') do
      post locals_url, params: { local: { name: @local.name, nbPlace: @local.nbPlace, typeService: @local.typeService } }, as: :json
    end
  ...
  test "should update local" do
    patch local_url(@local), params: { local: { name: @local.name, nbPlace: @local.nbPlace, typeService: @local.typeService } }, as: :json
    assert_response 200
  end
